...
## **Solution 1: Judge Sign Up**

Hackathon website has customized forms for judges to sign up that collects and stores all the necessary information for the admin. Below are all the fields we have the judges fill out. We sort out the judges based on their affiliations and use that data for our system. This highly increased efficiency and reduced possible errors for the hackathon committees.
...
## **Solution 2: The Online Judging Service**

Hackathon website implemented an effective online judging service in place of the manual judging system. It is essentially the same routine with timed rounds but it streamlines the entire assignment process. First both teams and judges data is pulled by the service using api.
...
RoseHack'24 was the first hackathon to use this service. Starting from smoother operations to easier judge handling, it led to the event successfully finishing more than 30 minutes early. Then the subsequent hackathons like Bear Hack, Designverse, Cutiehack all saved significant time and energy during their judging events.
